...
class DeformableFeatureAggregationPrev(BaseModule):
    def __init__(
        self,
        embed_dims: int = 256,
        num_groups: int = 8,
        ...
        super(DeformableFeatureAggregationPrev, self).__init__()
    ...
    def prepare_metas(self, metas, tensor):
        ...
        projection_mat = torch.from_numpy(np.array(
            projection_mat)).to(tensor.device, tensor.dtype)
        ida_mat = torch.from_numpy(np.array(
            ida_mat)).to(tensor.device, tensor.dtype)

        ida_mat = ida_mat.squeeze(1).squeeze(1)
        ...
        projection_mat = torch.matmul(projection_mat, ida_mat)

        bs, N, H, W = projection_mat.shape

        image_wh = torch.from_numpy(np.array(image_wh)).to(tensor.device, tensor.dtype)  # [224, 288, 3]
        image_wh = image_wh.unflatten(0, (bs, N))[..., [1, 0]].reshape(bs, N, 2)  # [288., 224.]
    ...
    def feature_sampling(
        ...
        points_2d = DeformableFeatureAggregationPrev.project_points(
...
# @MODELS.register_module()
# class SparseGaussian3DKeyPointsGeneratorPrev(BaseModule):
#     def __init__(
#         self,
#         embed_dims=256,
#         num_learnable_pts=0,
...
#         scale_range=None,
#     ):
#         super(SparseGaussian3DKeyPointsGeneratorPrev, self).__init__()
...
#         xyz = xyz.reshape(-1, nyu_pc_range.shape[0], 3)
...
#         xyz = xyz.reshape(bs, num_anchor, 3)
