...
class SparseGaussianFormerOnline(BaseModule):
    def __init__(
        self,
        anchor_encoder,
        norm_layer: dict,
        ...
        self.op_config_map = {
            ...
            "refine": refine_layer,
            "confidence_refine": confidence_refine_layer,
            "frozen_refine": frozen_refine_layer,
    ...
    def forward(
        ...
        prediction = []
        instance_feature_cache = instance_feature
        for i, op in enumerate(self.operation_order):
            if op == 'spconv':
            ...
            # elif "refine" in op:
                ...
                if i == len(self.operation_order) - 1:
                    instance_feature_cache = instance_feature
            elif op == "confidence_refine":
                if self.with_unc:
                    anchor, gaussian, cls, unc = self.layers[i](
                        instance_feature,
                        anchor,
                        anchor_embed,
                        metas,
        ...
        if self.with_unc:
            return prediction[-1], instance_feature_cache, unc
        else:
            return prediction[-1], instance_feature_cache, None