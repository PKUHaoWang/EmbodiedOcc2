...
class SparseGaussian3DDeltaForzenRefinementModule(BaseModule):
    ...
    def forward(
        ...
        anchor_updata_ratio[anchor_updata_ratio!=1] = 0
...
@MODELS.register_module()
class SparseGaussian3DDeltaForzenRefinementModuleUnc(BaseModule):
    def __init__(
        self,
        embed_dims=256,
        pc_range=None,
        ...
        super(SparseGaussian3DDeltaForzenRefinementModuleUnc, self).__init__()
        self.embed_dims = embed_dims
        self.output_dim = 10 + int(include_opa) + semantic_dim + int(include_v) * 2
        self.semantic_start = 10 + int(include_opa)
        self.semantic_dim = semantic_dim
    ...
    def forward(
        ...
        # 创建mask，只处理需要更新的样本（updata_ratio=1）
        update_mask = anchor_updata_ratio == 1
        update_indices = torch.where(update_mask)[1]  # 获取需要更新的索引
        
        # 初始化最终输出，使用anchor作为基础
        final_output = anchor.clone()
        
        if len(update_indices) > 0:
            # 只对需要更新的样本进行处理
            filtered_output = output[:, update_indices]
            filtered_anchor = anchor[:, update_indices]
            filtered_ratio = anchor_updata_ratio[:, update_indices]
            
            if self.restrict_xyz:
                delta_xyz_sigmoid = filtered_output[..., :3]
                delta_xyz_prob = 2 * safe_sigmoid(delta_xyz_sigmoid) - 1
                delta_xyz = torch.stack([
                    delta_xyz_prob[..., 0] * self.unit_sigmoid[0],
                    delta_xyz_prob[..., 1] * self.unit_sigmoid[1],
                ...
                filtered_output = torch.cat([delta_xyz, filtered_output[..., 3:]], dim=-1)
            
            if len(self.refine_state) > 0:
                refined_part_output = filtered_output[..., self.refine_state] * filtered_ratio + filtered_anchor[..., self.refine_state]
                filtered_output = torch.cat([refined_part_output, filtered_output[..., len(self.refine_state):]], dim=-1)
                
            delta_scale = filtered_output[..., 3:6]
            scale_final = filtered_anchor[..., 3:6] + delta_scale * filtered_ratio
            filtered_output = torch.cat([filtered_output[..., :3], scale_final, filtered_output[..., 6:]], dim=-1)

            rot = torch.nn.functional.normalize(filtered_output[..., 6:10], dim=-1)
            delta_w1, delta_x1, delta_y1, delta_z1 = rot[..., 0], rot[..., 1], rot[..., 2], rot[..., 3]
            # 在这里我们不需要额外的条件判断，因为所有filtered样本的ratio都是1
            
            w1, x1, y1, z1 = filtered_anchor[..., 6], filtered_anchor[..., 7], filtered_anchor[..., 8], filtered_anchor[..., 9]
            w_final = delta_w1 * w1 - delta_x1 * x1 - delta_y1 * y1 - delta_z1 * z1
            x_final = delta_w1 * x1 + delta_x1 * w1 + delta_y1 * z1 - delta_z1 * y1
            y_final = delta_w1 * y1 - delta_x1 * z1 + delta_y1 * w1 + delta_z1 * x1
            z_final = delta_w1 * z1 + delta_x1 * y1 - delta_y1 * x1 + delta_z1 * w1
            ...
            rot_final = torch.cat([w_final, x_final, y_final, z_final], dim=-1)
            rot_final = torch.nn.functional.normalize(rot_final, dim=-1)
            filtered_output = torch.cat([filtered_output[..., :6], rot_final, filtered_output[..., 10:]], dim=-1)
            
            delta_opa = filtered_output[..., 10: (10 + int(self.include_opa))]
            opa_final = filtered_anchor[..., 10: (10 + int(self.include_opa))] + delta_opa * filtered_ratio
            filtered_output = torch.cat([filtered_output[..., :10], opa_final, filtered_output[..., (10 + int(self.include_opa)):]], dim=-1)
            
            delta_semantics = filtered_output[..., self.semantic_start: (self.semantic_start + self.semantic_dim)]
            semantics_final = filtered_anchor[..., self.semantic_start: (self.semantic_start + self.semantic_dim)] + delta_semantics * filtered_ratio
            filtered_output = torch.cat([filtered_output[..., :self.semantic_start], semantics_final, filtered_output[..., (self.semantic_start + self.semantic_dim):]], dim=-1)
            
            # 将处理后的结果放回最终输出
            # 分批处理，避免维度不匹配问题
            for batch_idx in range(final_output.shape[0]):
                final_output[batch_idx, update_indices] = filtered_output[batch_idx]
