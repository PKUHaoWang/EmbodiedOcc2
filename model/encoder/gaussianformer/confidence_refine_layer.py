...
class SparseGaussian3DDeltaConfidenceRefinementModule(BaseModule):
    ...
    def forward(
        ...
        # import pdb; pdb.set_trace()
        # gs_semantic_logits = anchor[..., self.semantic_start: (self.semantic_start + self.semantic_dim)]
        # gs_semantic_label = torch.argmax(gs_semantic_logits, dim=-1)
        # stable_mask = (gs_semantic_label == 1) | (gs_semantic_label == 2) | (gs_semantic_label == 3)
        # old_mask = anchor_updata_ratio==0.5
        # stable_mask = stable_mask.unsqueeze(-1)
        # anchor_updata_ratio[old_mask & stable_mask] = 0.3
        # anchor_updata_ratio[old_mask & ~stable_mask] = 0.7
