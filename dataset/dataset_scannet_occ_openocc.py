...
class Scannet_Scene_OpenOccupancy_Dataset(data.Dataset):
    def __init__(
        ...
        # self.used_subscenes = [self.used_subscenes[1]]
    ...
    def __getitem__(self, index):
        ...
        normal_path = re.sub(r'posed_images(.*?)\.jpg$', r'normals\1.npy', rgb_path)
        if nio_enable:
            normal = torch.from_numpy(np.load(nio.download_cache_as_mem_file(normal_path)))
        else:
            normal = torch.from_numpy(np.load(normal_path))
        meta['normals'] = normal
        
        kappa_path = re.sub(r'posed_images(.*?)\.jpg$', r'kappas\1.npy', rgb_path)
        if nio_enable:
            kappa = torch.from_numpy(np.load(nio.download_cache_as_mem_file(kappa_path)))
        else:
            kappa = torch.from_numpy(np.load(kappa_path))
        meta['kappas'] = kappa
        
        depth_anything_path = re.sub(r'posed_images(.*?)\.jpg$', r'depthanything\1.npy', rgb_path)
        # print(depth_anything_path)
        if nio_enable:
            depth_anything_pre = torch.from_numpy(np.load(nio.download_cache_as_mem_file(depth_anything_path)))
        else:
            depth_anything_pre = torch.from_numpy(np.load(depth_anything_path, allow_pickle=True))
        meta['depth_anything_pre'] = depth_anything_pre
